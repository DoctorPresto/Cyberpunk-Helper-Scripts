...
REM Returns 0 if OK, 1 if too old or not found.
    ...
    REM Query the registry for the installed version
    set "REG_KEY=HKEY_CLASSES_ROOT\Installer\Dependencies\VC,redist.x64,amd64,14.42,bundle"
    for /f "tokens=2,*" %%A in (
        'reg query "%REG_KEY%" /v Version 2^>nul ^| find "Version"'
    ) do (
        set "INST_VERSION=%%B"
    )
